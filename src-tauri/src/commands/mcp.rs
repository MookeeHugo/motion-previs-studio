...
pub async fn start_mcp_server(project_id: String) -> Result<McpStatus, String> {
    ...
    if servers.is_none() { *servers = Some(HashMap::new()); }
    let servers = servers.as_mut().unwrap();
...
pub async fn stop_mcp_server(project_id: String) -> Result<(), String> {
    let child = { let mut s = MCP_SERVERS.lock().unwrap(); if let Some(ref mut s) = *s { s.remove(&project_id) } else { None } };
...
pub fn get_mcp_status(project_id: Option<String>) -> Result<Vec<McpStatus>, String> {
    let s = MCP_SERVERS.lock().unwrap();
    let s = s.as_ref().ok_or("Not initialized")?;
    if let Some(id) = project_id { return Ok(vec![McpStatus { running: s.contains_key(&id), project_id: Some(id) }]); }
